...
public class EbankingBackendApplication {
    ...
    CommandLineRunner usrs_(AccountService securityService) {
        return args -> {
            ...
            securityService.saveNewUser(new AppUser(null, "saadLAAUMARI", "1234", new ArrayList<>()));
            securityService.saveNewUser(new AppUser(null, "Saad", "2027", new ArrayList<>()));

            securityService.addRoleToUser("Saad", "USER");
            securityService.addRoleToUser("saadLAAUMARI", "USER");
            securityService.addRoleToUser("saadLAAUMARI", "ADMIN");
    ...
    CommandLineRunner commandLineRunner(BankAccountService bankAccountService){
        return args -> {
            Stream.of("Saad","Ahmed","Mohammed").forEach(
                    nom -> {
